...
void clcd_write(unsigned char byte, unsigned char control_bit)
{
	...
	PORT_DIR = INPUT;//rd7 input
	CLCD_RW = HI;//read the data
	CLCD_RS = INSTRUCTION_COMMAND;

	do//generating clock pulse untill int=struction is written
	{
		CLCD_EN = HI;
		CLCD_EN = LO;
	} while (CLCD_BUSY);
...
{
	clcd_write(addr, INSTRUCTION_COMMAND);//where we want to display
	clcd_write(data, DATA_COMMAND);//what we want to display
}
